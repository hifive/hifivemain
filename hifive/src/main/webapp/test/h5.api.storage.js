...
test('local.setItem / local.getItem', function() {
	if (!h5.api.storage.isSupported) {
		ok(false, 'このブラウザはWeb Storageをサポートしていません。');
		return;
	}
...
test('local.removeItem', function() {
	if (!h5.api.storage.isSupported) {
		ok(false, 'このブラウザはWeb Storageをサポートしていません。');
		return;
	}
...
test('session.setItem / session.getItem', function() {
	if (!h5.api.storage.isSupported) {
		ok(false, 'このブラウザはWeb Storageをサポートしていません。');
		return;
	}
...
test('session.removeItem', function() {
	if (!h5.api.storage.isSupported) {
		ok(false, 'このブラウザはWeb Storageをサポートしていません。');
		return;
	}
	...
	strictEqual(h5.api.storage.session.getItem('test2'), null, 'キー=test2 は削除されているのでnullであること。');
});

test('session.clear', function() {
	if (!h5.api.storage.isSupported) {
		ok(false, 'このブラウザはWeb Storageをサポートしていません。');
		return;
	}
