...
(function() {
	...
	asyncTest('デバッグモード ダミー位置情報をセットしていないときは元のAPIを使うこと。エラーが起きるか位置が取得できる。', 1, function() {
		if (!isSupported) {
			expect(1);
	...
	asyncTest('デバッグモード ダミー位置情報がセットした通りに取得できること。', 8, function() {
		// ダミー位置をセットする
		var params = {
			coords: {
	...
	asyncTest('デバッグモード ダミー位置情報セット時に省略したパラメータにデフォルト値が入って取得できること。', 24, function() {
		var params = [{}, {
			coords: {
	...
	asyncTest('デバッグモード 失敗 強制エラー', 1, function() {
	...
	asyncTest('watchPosition()を1000ms、500ms、2500msとタイムアウトを変えて3回実行し、それぞれから位置情報を取得後、正常に停止できるか。', 61,
			function() {
				// 1秒ごと
				h5.dev.api.geo.watchIntervalTime = 1000;
				var paramsArray = [{
	...
	module('H5Api - Geo Location (Debug Mode) - getDistance', {
		setup: function() {
			h5.dev.api.geo.watchIntervalTime = 1000;
			h5.dev.api.geo.dummyPositions = [];
			h5.dev.api.geo.forceError = false;
