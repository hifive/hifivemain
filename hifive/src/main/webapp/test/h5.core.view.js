...
(function() {
					...
					for ( var i = 0, l = result.length; i < l; i++) {
						...
						if (obj.path === './template/test8.ejs') {
							...
							ok(obj.absoluteUrl.search(/http:\/\//) === 0
									...
									'絶対パスが取得できること(http://～～/template/test8.ejs): '
											+ obj.absoluteUrl);
							...
							h5.core.view
									...
									.fail(
											function(e) {
												...
												strictEqual(h5.core.view.isAvailable('inscript'),
														true);
	...
	asyncTest('isAvailable() ロードしたテンプレートIDがキャッシュに存在することを確認。', 8,
			function() {
	...
	asyncTest('clear() テンプレートをキャッシュから全て削除。', 8,
			function() {
				var loadedIds = ['template2', 'template3', 'template4', 'template5'];
				var view = h5.core.view.createView();
				var p = view.load(['./template/test2.ejs', './template/test3.ejs',
	...
	asyncTest('clear() テンプレートIDを指定してキャッシュからテンプレート削除。', 12,
			function() {
				var loadedIds = ['template2', 'template3', 'template4', 'template5'];
				...
				p.done(function(result) {
					...
					for ( var i = 0, l = existIds.length; i < l; i++) {
						var id = existIds[i];
						ok(view.isAvailable(id), '削除されていないテンプレートはisAvailable(id)の結果がfalseであること。'
								+ id);
						try {
							view.get(id);
