...
(function() {
	...
	$(function() {
		...
		asyncTest(
				'bind() 引数が不正、またはコントローラ化されたコントローラからの呼び出しでない場合、及び指定された要素が存在しないまたは、複数ある場合にエラーが出ること',
				7, function() {
					$('#controllerTest').append('<div class="test">a</div>');
					$('#controllerTest').append('<div class="test">b</div>');
					var controller = {
		...
		asyncTest(
				'getAllControllers() 現在バインドされているすべてのコントローラを取得できること',
				function() {
					...
					setTimeout(
							function() {
								for ( var i = 0; i < 3; i++) {
									var prop = ['b', 'a', 'test'][i];
									var str = ['孫', '子', '親'][i];
									ok(isDisposed(disposedController[prop]), str
											+ 'コントローラがdisposeされていること');
								}
								start();
							}, 0);
				},
		...
		asyncTest(
				'コントローラの初期化処理中にエラーが起きた時のcommonFailHandlerの動作 ルートコントローラのreadyPromiseにfailハンドラを登録した場合',
				2, function() {
					var cfh = 0;
					h5.settings.commonFailHandler = function() {
						cfh++;
		...
		asyncTest('テンプレートのカスケーディング1', function() {
			...
			testController.readyPromise.done(function() {
				...
				deepEqual(errorObj.code, ERR_VIEW.ERR_CODE_TEMPLATE_ID_UNAVAILABLE,
		...
		asyncTest('テンプレートのカスケーディング2', function() {
			...
			testController.readyPromise.done(function() {
				...
				ok(html != null && html.length > 0,
						'ビューがカスケードした場合でもターゲットは自身のrootElementからfindしているか');
				testController.unbind();
			});
		});
		...
		asyncTest('this.indicator()', function() {
			...
			var controllerBase = {
				...
				'input[type=button] click': function() {
					...
					strictEqual($(indicator.target).find(
							'.blockUI.a.blockElement > .indicator-message').text(),
							'BlockMessageTest');
		...
		asyncTest('this.triggerIndicator() triggerIndicator()でグローバルインジケータを表示', 5,
				function() {
					var testController = null;
					var controllerBase = {
						__name: 'TestController',
						...
						'input[type=button] click': function() {
							...
							strictEqual($(indicator.target).find(
									'.blockUI.a.blockPage > .indicator-message').text(),
									'BlockMessageTest');
							strictEqual($(indicator.target).find('.blockUI.blockOverlay').length,
									1, 'Indicator#show() インジケータが表示されること');
							strictEqual($(indicator.target).find('.throbber-percent').text(), '20',
									'Indicator#show() 進捗率が表示されること');

							strictEqual($(indicator.target).find('.blockUI.blockOverlay').css(
		...
		asyncTest('this.indicator() オプションにプレーンオブジェクト以外を渡した時は無視されること', 4, function() {
			...
			var controllerBase = {
				...
				'input[type=button] click': function() {
					...
					deepEqual($(indicator.target).find(
							'.blockUI.a.blockElement > .indicator-message').text(), '',
		...
		asyncTest('h5.ui.indicator()', function() {
			...
			var controllerBase = {
				...
				'input[type=button] click': function() {
					...
					strictEqual($(indicator2.target).find(
							'.blockUI.a.blockPage > .indicator-message').text(),
							'BlockMessageTest2');
			...
			var controllerBase = {
				...
				deferredFunc: function() {
					...
					setTimeout(function() {
						...
						setTimeout(function() {
							strictEqual($(indicator.target).find('.blockUI.blockOverlay').length,
									0, 'resolve()が呼ばれると、インジケータが非表示になること');
							start();
						}, 0);
					}, 0);
			...
			var controllerBase = {
				...
				'input[type=button] click': function() {
					...
					setTimeout(function() {
						...
						strictEqual($(indicator2.target).find('.blockUI.blockOverlay').css(
								'display'), 'block', 'オーバーレイが表示されていること');
		...
		asyncTest('h5.ui.indicator() テーマを変更して実行', function() {
			...
			var controllerBase = {
				...
				'input[type=button] click': function() {
					...
					strictEqual($(indicator2.target).find(
							'.blockUI.b.blockPage > .indicator-message').text(),
							'BlockMessageTest2');
				...
				function() {
					...
					var controllerBase1 = {
						...
						'input click': function(context) {
							...
							strictEqual(context.SELECTOR_TYPE_LOCAL, 1,
									'selectorTypeを表す定数が格納されていること 1');
		...
		asyncTest('__construct, __init, __readyで子コントローラに親コントローラのインスタンスを持たせた時に無限ループにならないか',
				function() {
		...
		asyncTest('初期化パラメータを渡せるか',
				function() {
					...
					rootController.readyPromise
							.done(function() {
								start();

								ok(args !== rConstruct,
										'__constructでルートコントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === rConstruct.param,
										'__constructでルートコントローラに渡された初期化パラメータのプロパティは正しいか');
								ok(args !== rInit, '__initでルートコントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === rInit.param,
										'__initでルートコントローラに渡された初期化パラメータのプロパティは正しいか');
								ok(args !== rReady, '__readyでルートコントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === rReady.param,
										'__readyでルートコントローラに渡された初期化パラメータのプロパティは正しいか');

								ok(args !== pConstruct,
										'__constructで子コントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === pConstruct.param,
										'__constructで子コントローラに渡された初期化パラメータのプロパティは正しいか');
								ok(args !== pInit, '__initで子コントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === pInit.param,
										'__initで子コントローラに渡された初期化パラメータのプロパティは正しいか');
								ok(args !== pReady, '__readyで子コントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === pReady.param,
										'__readyで子コントローラに渡された初期化パラメータのプロパティは正しいか');


								ok(args !== cConstruct,
										'__constructで孫コントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === cConstruct.param,
										'__constructで孫コントローラに渡された初期化パラメータのプロパティは正しいか');
								ok(args !== cInit, '__initで孫コントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === cInit.param,
										'__initで孫コントローラに渡された初期化パラメータのプロパティは正しいか');
								ok(args !== cReady, '__readyで孫コントローラに渡された初期化パラメータの参照は変わっているか');
								ok(args.param === cReady.param,
										'__readyで孫コントローラに渡された初期化パラメータのプロパティは正しいか');
		...
		asyncTest('enableListeners() / disableListeners() の動作', function() {
			...
			c.readyPromise.done(function() {
				...
				root.trigger('childCustomEvent');
				ok(ret === 100,
						'親のenableListeners()によって、useHandlersがtrueである子コントローラのイベントハンドラが動作しているか');
				root.trigger('parentCustomEvent');
				...
				function() {
					var testController = {
						...
						__ready: function(context) {
							...
							try {
								this
										.throwError('コントローラ"{0}"における{1}のテスト', this.__name,
												'throwError');
							} catch (e) {
							...
							try {
								...
								ok(e, 'エラータイプのみ指定してthrowCustomErrorメソッドを実行すると、エラーが投げられているか');
								strictEqual(e.customType, 'customType',
		...
		test('あるセレクタに対して重複するイベントハンドラを設定した時の動作', 1,
				function() {
					var testController = {
						__name: 'TestController',
						' {rootElement}   click': function(context) {},
					...
					try {
						...
						strictEqual(e.code, ERR.ERR_CODE_SAME_EVENT_HANDLER,
				...
				function() {
					...
					var controller = {
						...
						'{rootElement} h5trackstart': function(context) {
							...
							strictEqual(event.pageX, 10,
									'h5trackstartイベントのEventオブジェクトにpageXは設定されているか');
							strictEqual(event.pageY, 10,
									'h5trackstartイベントのEventオブジェクトにpageYは設定されているか');
							...
							ok(event.offsetX != null,
									'h5trackstartイベントのEventオブジェクトにoffsetXは設定されているか');
							ok(event.offsetY != null,
									'h5trackstartイベントのEventオブジェクトにoffsetYは設定されているか');
						},
						...
						'{rootElement} h5trackmove': function(context) {
							...
							var event = context.event;
							strictEqual(event.pageX, 15,
									'h5trackmoveイベントのEventオブジェクトにpageXは設定されているか');
							strictEqual(event.pageY, 15,
									'h5trackmoveイベントのEventオブジェクトにpageYは設定されているか');
							...
							ok(event.offsetX != null,
									'h5trackmoveイベントのEventオブジェクトにoffsetXは設定されているか');
							ok(event.offsetY != null,
									'h5trackmoveイベントのEventオブジェクトにoffsetYは設定されているか');
						...
						'{rootElement} h5trackend': function(context) {
							...
							var event = context.event;
							strictEqual(event.pageX, 20,
									'h5trackendイベントのEventオブジェクトにpageXは設定されているか');
							strictEqual(event.pageY, 20,
									'h5trackendイベントのEventオブジェクトにpageYは設定されているか');
		...
		asyncTest(
				'h5trackイベント(touchstart, touchmove, touchend)',
				26,
				function() {
					var controller = {
