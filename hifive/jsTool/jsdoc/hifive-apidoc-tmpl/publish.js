(function() {
	// binフォルダから見たテンプレートフォルダのパス
	__dirname = '../';
	// テンプレート名 (=テンプレートを格納しているフォルダ名)
	var TMPL_NAME = 'hifive-apidoc-tmpl';

	var _ = require('underscore/underscore'),template = require('underscore/template'),fs = require('fs'),helper = require('jsdoc/util/templateHelper'),scopeToPunc = {
		'static': '.',
		'inner': '~',
		'instance': '#'
	};
	...
	publish = function(data, opts, tutorials) {
		var out = '',containerTemplate = template.render(fs.readFileSync(__dirname
				+ '/' + TMPL_NAME + '/tmpl/container.tmpl')),tutorialTemplate = template.render(fs
				.readFileSync(__dirname + '/' + TMPL_NAME + '/tmpl/tutorial.tmpl'));
		...
		function render(tmpl, partialData) {
			...
			if (!renderFunction) {
				renderFunction = arguments.callee.cache[tmpl] = template.render(fs
						.readFileSync(__dirname + '/' + TMPL_NAME + '/tmpl/' + tmpl));
			}
		...
		function addAttribs(f) {
			...
			f.attribs = '<span class="type-signature">'
					+ htmlsafe(attribs.length ? '<' + attribs.join(', ') + '> ' : '') + '</span>';
		}
		...
		var fromDir = __dirname + '/' + TMPL_NAME + '/static',staticFiles = fs.ls(fromDir, 3);

		staticFiles.forEach(function(fileName) {
			// staticフォルダの中身をコピー
			var toDir = fs.toDir(fileName.replace(/.*\/static/, outdir));
			fs.mkPath(toDir);
		...
		function linkto(longname, linktext) {
			// 配列に対してもリンクを探すように変更
			var url = helper.longnameToUrl[longname.replace('[]', '')];
			return url ? '<a href="' + url + '">' + (linktext || longname) + '</a>'
					: (linktext || longname);
		}
		...
		var moduleNames = find({
			kind: 'module'
		});
		if (moduleNames.length) {
			nav += '<h3>Modules</h3><ul>';
			moduleNames.forEach(function(m) {
				if (!seen.hasOwnProperty(m.longname))
		...
		if (classNames.length) {
			...
			classNames.forEach(function(c) {
				...
				if (!seen.hasOwnProperty(c.longname))
		...
		var namespaceNames = find({
			kind: 'namespace'
		});
		if (namespaceNames.length) {
			nav += '<h3>Namespaces</h3><ul>';
			namespaceNames.forEach(function(n) {
				if (!seen.hasOwnProperty(n.longname))
		...
		for ( var longname in helper.longnameToUrl) {
			...
			if (namespaces.length)
				generate('Namespace: ' + namespaces[0].name, namespaces,
						helper.longnameToUrl[longname]);

			// var constants = find({kind: 'constant', longname: longname});
			// if (constants.length) generate('Constant: '+constants[0].name, constants,
			// helper.longnameToUrl[longname]);
			...
			if (externals.length)
				generate('External: ' + externals[0].name, externals,
						helper.longnameToUrl[longname]);
		}
		...
		function generate(title, docs, filename) {
			...
			var path = outdir + '/' + filename,html = containerTemplate.call(data, data);

			html = helper.resolveLinks(html); // turn {@link foo} into <a
			// href="foodoc.html">foo</a>
		...
		function generateTutorial(title, tutorial, filename) {
			...
			var path = outdir + '/' + filename,html = tutorialTemplate.call(data, data);
		...
		function saveChildren(node) {
			node.children.forEach(function(child) {
				generateTutorial('Tutorial: ' + child.title, child, helper
						.tutorialToUrl(child.name));
			});
		}
